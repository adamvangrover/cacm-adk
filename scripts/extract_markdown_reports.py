...
INPUT_JSONL_FILE = os.path.join(PROJECT_ROOT, "default_synthetic_library_v1", "synthetic_reports.jsonl")
OUTPUT_REPORTS_DIR = os.path.join(PROJECT_ROOT, "default_synthetic_library_v1", "human_readable_reports")

def sanitize_filename(name):
    """ Sanitize a string to be used as a valid filename. """
    name = str(name) # Ensure name is a string
    name = re.sub(r'[^\w\s-]', '', name).strip().lower() # Remove non-alphanumeric (except underscore, hyphen), strip, lowercase
...
def main():
    ...
    if not os.path.exists(INPUT_JSONL_FILE):
        ...
        print("Please ensure `scripts/generate_synthetic_library.py` has been run successfully first.")
        return
            ...
            for line_number, line in enumerate(f_jsonl, 1):
                ...
                try:
                    ...
                    if markdown_report is None:
                        print(f"WARNING: Line {line_number} for company '{company_id}' has no 'generated_markdown_report' field. Skipping.")
                        error_count += 1
                        continue
                    ...
                    print(f"ERROR: Could not parse JSON on line {line_number}: {e_json}. Line content (first 100 chars): '{stripped_line[:100]}'")
                    error_count += 1
                except KeyError as e_key: # Should ideally not happen if JSON structure is consistent
                    print(f"ERROR: Missing expected key {e_key} on line {line_number} for company '{current_company_id_for_error}'.")
                    error_count += 1
