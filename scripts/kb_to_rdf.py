...
def main():
    ...
    if "financialFormulas" in kb_data:
        for formula in kb_data["financialFormulas"]:
            ...
            if not formula_id:
                print(
                    f"Warning: Skipping formula due to missing 'formulaId': {formula.get('name')}"
                )
                continue
            ...
            if formula.get("description"):
                g.add(
                    (
                        subject_uri,
                        RDFS.comment,
                        Literal(formula["description"], lang="en"),
                    )
                )
            if formula.get("calculation"):
                g.add(
                    (
                        ...
                        Literal(formula["calculation"], datatype=XSD.string),
                    )
                )
            ...
            for inp in inputs:
                g.add(
                    (
                        subject_uri,
                        KGPROP.hasInputLiteral,
                        Literal(inp, datatype=XSD.string),
                    )
                )
            ...
            if formula.get("output"):
                g.add(
                    (
                        subject_uri,
                        KGPROP.hasOutputLiteral,
                        Literal(formula["output"], datatype=XSD.string),
                    )
                )
    ...
    if "riskFactors" in kb_data:
        for rf in kb_data["riskFactors"]:
            ...
            if not rf_id:
                print(
                    f"Warning: Skipping risk factor due to missing 'riskFactorId': {rf.get('name')}"
                )
                continue
            ...
            if rf.get("description"):
                g.add(
                    (subject_uri, RDFS.comment, Literal(rf["description"], lang="en"))
                )
            ...
            for strat in strategies:
                g.add(
                    (
                        subject_uri,
                        KGPROP.hasMitigationStrategy,
                        Literal(strat, datatype=XSD.string),
                    )
                )
            ...
            for ind in industries:
                g.add(
                    (
                        subject_uri,
                        KGPROP.appliesToIndustryLiteral,
                        Literal(ind, datatype=XSD.string),
                    )
                )
    ...
    if "macroeconomicIndicators" in kb_data:
        for indicator in kb_data["macroeconomicIndicators"]:
            ...
            if not indicator_id:
                print(
                    f"Warning: Skipping indicator due to missing 'indicatorId': {indicator.get('name')}"
                )
                continue
            ...
            if indicator.get("description"):
                g.add(
                    (
                        subject_uri,
                        RDFS.comment,
                        Literal(indicator["description"], lang="en"),
                    )
                )
            if indicator.get("source"):
                ...
                g.add(
                    (
                        subject_uri,
                        DCTERMS.source,
                        Literal(indicator["source"], lang="en"),
                    )
                )
