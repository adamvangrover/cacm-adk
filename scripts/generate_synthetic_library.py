...
def get_company_specific_texts(company_id):
    ...
    if company_id == "MSFT":
        ...
        return {
            "business_overview": GENERIC_BUSINESS_OVERVIEW_TEMPLATE.format(
                company_id=company_id
            )
            + " (TESTCORP specific details would be here if available.)",
            "risk_factors": GENERIC_RISK_FACTORS_TEMPLATE.format(company_id=company_id)
            + " (TESTCORP specific risks would be here.)",
        }
...
def construct_cacm_for_company(company_id: str, company_texts: dict):
    ...
    cacm_instance = {
        ...
        "inputs": {
            ...
            "user_faa_summary_guidance": {
                "value": "",
                "description": "User guidance for FAA summary - default for library.",
            },
            "user_dcf_discount_rate": {
                "value": None,
                "description": "User DCF discount rate override - default for library.",
            },
            "user_dcf_terminal_growth_rate": {
                "value": None,
                "description": "User DCF terminal growth override - default for library.",
            },
        },
        ...
        "workflow": [  # Copied & adapted from msft_comprehensive_analysis_workflow.json
            ...
            },
            {
                ...
                "outputBindings": {
                    "final_report_text": "cacm.outputs.final_generated_report"
                },
            },
        ],
    }
...
def assemble_full_report_object(
    company_id: str,
    company_name_processed: str,
    workflow_id_used: str,
    cacm_inputs: dict,
    ...
    report_obj = {
        ...
        "inputs_to_workflow": {  # Capture key varying inputs
            ...
            "user_faa_summary_guidance": cacm_inputs["user_faa_summary_guidance"][
                "value"
            ],
            "user_dcf_discount_rate": cacm_inputs["user_dcf_discount_rate"]["value"],
            "user_dcf_terminal_growth_rate": cacm_inputs[
                "user_dcf_terminal_growth_rate"
            ]["value"],
        },
        "ingested_data_summary": workflow_outputs.get(
            "intermediate.step1_ingestion_summary",
            ...
            {
                "status": "info",
                "message": "Ingestion summary not directly bound in workflow outputs.",
            },
        ),
...
async def main():
    ...
    if orch.validator is None:
        from cacm_adk_core.validator.validator import (
            Validator,
        )  # Assuming Validator can be init'd simply
    ...
    for company_id in TARGET_COMPANIES:
        ...
        workflow_id, cacm_instance_data = construct_cacm_for_company(
            company_id, company_specific_texts
        )
        ...
        if success:
            ...
            company_name_in_workflow = cacm_instance_data["workflow"][0][
                "inputBindings"
            ]["companyName"]["value"]
