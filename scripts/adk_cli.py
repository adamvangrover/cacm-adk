...
def get_orchestrator(validator_instance):
    if validator_instance is None:
        return None  # Already handled by get_validator
    if not os.path.exists(DEFAULT_CATALOG_PATH):
        click.echo(
            f"Error: Compute Capability Catalog not found at {DEFAULT_CATALOG_PATH}. Orchestrator may not function fully.",
            err=True,
        )
...
def list_templates():
    ...
    if not os.path.isdir(engine.templates_dir):
        click.echo(
            f"Error: Templates directory not found: {engine.templates_dir}", err=True
        )
        return
...
@click.option(
    "--name", default=None, help="Name for the new CACM (overrides template)."
)
@click.option(
    "--description",
    default=None,
    help="Description for the new CACM (overrides template).",
)
# Add more override options as needed, e.g., --author, --tags (comma-separated)
def instantiate_template_cmd(
    template_filename, output_filepath, cacm_id, name, description
):
    """Instantiates a CACM template and saves it to a file."""
    engine = get_template_engine()
    if not os.path.isdir(
        engine.templates_dir
    ):  # Check again in case it was created after engine init
        click.echo(
            f"Error: Templates directory not found: {engine.templates_dir}", err=True
        )
        return
    ...
    instance = engine.instantiate_template(
        template_filename, cacm_id=cacm_id, overrides=overrides if overrides else None
    )
    ...
    if instance:
        try:
            ...
            click.echo(
                f"Successfully instantiated template '{template_filename}' to '{output_filepath}'"
            )
        ...
        click.echo(
            f"Error: Could not instantiate template '{template_filename}'. Check logs or template existence.",
            err=True,
        )
...
def validate_cacm_cmd(cacm_filepath):
    ...
    if not validator or not validator.schema:
        click.echo(
            "Error: Validator or schema could not be initialized. Cannot validate.",
            err=True,
        )
        return
    ...
    if is_valid:
        click.echo(
            click.style(
                f"CACM file '{cacm_filepath}' is VALID according to the schema.",
                fg="green",
            )
        )
    else:
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Directory to save generated reports. Overrides default path from agent.",
)
def run_cacm_cmd(cacm_filepath, output_dir):
    ...
    if not validator or not validator.schema:
        click.echo(
            "Error: Validator or schema could not be initialized for pre-run validation.",
            err=True,
        )
        return
    ...
    if not orchestrator.compute_catalog:  # Check if catalog loaded properly
        click.echo(
            "Warning: Compute capability catalog was not loaded. 'run' may provide limited capability checks.",
            err=True,
        )
    ...
    success, logs, outputs = asyncio.run(
        _run_cacm_async(orchestrator, cacm_instance_data)
    )
    ...
    if success:
        ...
        if outputs:
            ...
            for output_name, output_data in outputs.items():
                ...
                if (
                    ...
                    and "generated_report_text" in output_value
                    and "report_file_path" in output_value
                ):
                    report_content = output_value[
                        "generated_report_text"
                    ]  # Key updated
                    conceptual_file_path = output_value[
                        "report_file_path"
                    ]  # Key updated

                    click.echo(
                        f"  Found potential report package in output: '{output_name}'"
                    )
                    ...
                    try:
                        ...
                        click.echo(
                            click.style(
                                f"  Report from '{output_name}' saved to: {final_output_path}",
                                fg="green",
                            )
                        )
                    except IOError as e:
                        click.echo(
                            click.style(
                                f"  Error saving report from '{output_name}' to {final_output_path}: {e}",
                                fg="red",
                            ),
                            err=True,
                        )
                        ...
                        Exception
                    ) as e:  # Catch other potential errors like invalid paths
                        click.echo(
                            click.style(
                                f"  An unexpected error occurred while saving report from '{output_name}': {e}",
                                fg="red",
                            ),
                            err=True,
                        )
                    ...
                    click.echo(
                        f"  - {output_name}: (type: {type(output_value).__name__})"
                    )
        ...
        click.echo(
            click.style(
                "\nCACM workflow execution failed or did not complete due to validation/runtime errors.",
                fg="red",
            )
        )
