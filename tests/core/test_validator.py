...
class TestValidator(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not os.path.exists(schema_path):
             raise unittest.SkipTest(f"Schema file not found at resolved path: {schema_path}. Check SCHEMA_FILE_PATH and execution directory.")
        ...
        if not cls.validator.schema:
            ...
            raise unittest.SkipTest(f"Could not load schema at {schema_path}. Validator __init__ might have failed.")
    ...
    def test_validator_initialization_and_schema_loading(self):
        ...
        self.assertEqual(self.validator.schema.get("title"), "CACM Definition") # Check a known field
        self.assertEqual(self.validator.schema.get("description"), "Schema for a Credit Analysis Capability Module (CACM). Version 0.2")
    ...
    def test_invalid_cacm_missing_name(self):
        invalid_cacm = {
            ...
            "metadata": {"creationDate": "2023-01-01T12:00:00Z"},
            "inputs": { "dummy_input": {"description": "d", "type": "string"}}, # Made valid to isolate error
            "outputs": { "dummy_output": {"description": "d", "type": "string"}}, # Made valid
            "workflow": [{ "stepId": "s1", "description": "d", "computeCapabilityRef": "d"}] # Made valid
        }
        ...
        self.assertTrue(any("'name' is a required property" in error.get("message", "") for error in errors), f"Errors: {errors}")
    ...
    def test_invalid_input_item_missing_type(self):
        ...
        is_valid, errors = self.validator.validate_cacm_against_schema(invalid_cacm)
        self.assertFalse(is_valid, "CACM should be invalid due to input item missing 'type'.")
        self.assertTrue(any("'type' is a required property" in error.get("message", "") and error.get("path", []) == ["inputs", "bad_input"] for error in errors), f"Errors: {errors}")
    ...
    def test_invalid_workflow_step_missing_id(self):
        invalid_cacm = {
            ...
            "metadata": {"creationDate": "2023-01-01T12:00:00Z"},
            "inputs": { "dummy_input": {"description": "d", "type": "string"}}, # Made valid
            "outputs": { "dummy_output": {"description": "d", "type": "string"}}, # Made valid
            "workflow": [
        ...
        is_valid, errors = self.validator.validate_cacm_against_schema(invalid_cacm)
        self.assertFalse(is_valid, "CACM should be invalid due to workflow step missing 'stepId'.")
        # Path should be ['workflow', 0] because it's the first item in the array
        self.assertTrue(any("'stepId' is a required property" in error.get("message", "") and error.get("path", []) == ["workflow", 0] for error in errors), f"Errors: {errors}")
