...
class TestBaseAgent(unittest.IsolatedAsyncioTestCase):
    async def test_agent_initialization(self):
        agent = ConcreteTestAgent(agent_name="TestAgent", kernel_service=SimpleMockKernelService())
    ...
    async def test_get_or_create_agent(self):
        requesting_agent = ConcreteTestAgent(agent_name="RequestingAgent", kernel_service=SimpleMockKernelService())
        mock_manager = MagicMock()
        mock_manager.get_or_create_agent_instance = AsyncMock(return_value="mock_target")
        requesting_agent.set_agent_manager(mock_manager)
        target = await requesting_agent.get_or_create_agent("Target", context_data={"k": "v"})
        self.assertEqual(target, "mock_target")
        mock_manager.get_or_create_agent_instance.assert_called_with("Target", context_data_for_creation={"k": "v"})
...
class TestAnalysisAgent(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        def get_plugin(key):
            if key == "FinancialAnalysis": return self.mock_fin_plugin
            if key == "ReportingAnalysisSkills": return self.mock_report_plugin
            return None # Or raise KeyError to be stricter
        self.mock_kernel.plugins.get.side_effect = get_plugin
        self.mock_kernel.plugins.__getitem__.side_effect = get_plugin # For base_agent.get_plugin

        self.mock_fin_plugin.get.return_value = self.mock_ratio_func
        self.mock_report_plugin.get.return_value = self.mock_summ_func # All report skills point to one mock

        self.analysis_agent = AnalysisAgent(self.mock_kernel_service)
        self.shared_context = SharedContext("test_cacm")
        self.analysis_agent.set_agent_manager(AsyncMock()) # Basic mock for agent_manager

    async def test_run_successful_full_analysis(self):
        self.shared_context.set_data("financial_data_for_ratios_expanded", {"current_assets": 100})
        self.shared_context.set_data("structured_financials_for_summary", {"revenue": 100})
    ...
    async def test_run_summarization_plugin_not_found(self):
        self.shared_context.set_data("financial_data_for_ratios_expanded", {"current_assets": 1}) # Ratio part will run
        self.mock_kernel.plugins.get.side_effect = lambda key: self.mock_fin_plugin if key == "FinancialAnalysis" else None # ReportingAnalysisSkills not found

        # Ratio skill needs a mock return value for this path
        ratio_payload = {"calculated_ratios": {"current_ratio": 2.0}, "errors": []}
        self.mock_kernel.invoke.return_value = MagicMock(spec=FunctionResult, value=ratio_payload)
