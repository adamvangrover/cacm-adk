...
BASIC_TEMPLATE_FILENAME_JSON = "basic_ratio_analysis_template.json" # Using one of the actual templates
...
class TestTemplateEngineMinimal(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        src_basic_template_path = os.path.join(project_root, MAIN_TEMPLATES_LIB_DIR, BASIC_TEMPLATE_FILENAME_JSON)
        dest_basic_template_path = os.path.join(TEST_TEMPLATES_DIR_MINIMAL, BASIC_TEMPLATE_FILENAME_JSON)
        ...
        if os.path.exists(src_basic_template_path):
            ...
            print(f"Warning: Main template {src_basic_template_path} not found. Creating a minimal version for tests.")
            minimal_basic_template_content_dict = {
                "name": "Minimal Basic Template", "description": "A very basic template for testing.",
                "cacmId": "replace-me", "metadata": {"templateDetails": {"templateName": "Minimal Template"}}
            } # Pure JSON as dict
    ...
    def test_load_template_json(self): # Renamed to reflect JSON
        ...
        self.assertIsNotNone(template_data, f"Should load the template: {BASIC_TEMPLATE_FILENAME_JSON}.")
        # Check for a field that exists in both the full template and the minimal one
        self.assertIn("name", template_data) 
        if template_data.get("name") == "Minimal Basic Template": # Check if it's the minimal version
             ...
             self.assertEqual(template_data.get("name"), "Basic Financial Ratio Analysis")


    def test_list_templates_json(self): # Renamed
        templates = self.engine.list_templates()
        self.assertEqual(len(templates), 1, "Should list one .json template from the test directory.")
        if len(templates) == 1:
            ...
            loaded_template_for_name_check = self.engine.load_template(BASIC_TEMPLATE_FILENAME_JSON)
    ...
    def test_instantiate_template_json(self): # Renamed
        ...
        self.assertNotEqual(instance.get("cacmId"), "replace-me") # Minimal template has this
