...
try:
    ...
    print(f"Error importing Orchestrator: {e}. Current sys.path: {sys.path}", file=sys.stderr)
...
if Orchestrator:
    ...
    orchestrator_instance = None
    print("Orchestrator class not imported, API endpoint will not function.", file=sys.stderr)
...
async def run_basic_ratio_analysis_endpoint(input_data: BasicRatioAnalysisInput):
    if not orchestrator_instance:
        raise HTTPException(status_code=503, detail="Orchestrator service is not available. Check server logs.")
    ...
    if not os.path.exists(template_path):
        ...
        raise HTTPException(status_code=500, detail=f"CACM template file not found on server.")

    input_dict = {"financialStatementData": input_data.financialStatementData.model_dump()}
    ...
    try:
        ...
        response_payload = BasicRatioAnalysisOutput(
            calculated_ratios=RatioCalculationResult(**ratios_data) if ratios_data else None,
            execution_errors=errors_data
        )
        return response_payload

    except FileNotFoundError:
        raise HTTPException(status_code=500, detail="CACM template not found during execution.")
    except Exception as e:
