...
def handle_chatbot_command():
    ...
    try:
        ...
        if "java" in command_parts:  # Very basic parsing
            target_system = "java"
            c_program_invocation_args.extend(
                ["--target", "java", "--command", user_command]
            )
        elif "python" in command_parts:
            target_system = "python"
            c_program_invocation_args.extend(
                ["--target", "python", "--command", user_command]
            )
        else:
            ...
            app.logger.info(
                f"No specific target (java/python) detected in command: '{user_command}'. Passing full command to adk_controller."
            )
        ...
        app.logger.error(
            f"Chatbot backend error: {e}", exc_info=True
        )  # Log stack trace
