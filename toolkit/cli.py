...
def list_commands():
    ...
    click.echo(
        "- toolkit llm list-skills        : Lists available LLM/Semantic Kernel skills."
    )
    click.echo(
        "- toolkit llm invoke <p> <f>     : Invokes an LLM/Semantic Kernel skill."
    )
    click.echo("- toolkit kb list-classes        : Lists ontology classes.")
    click.echo(
        "- toolkit kb get-details <uri>   : Gets details for an ontology entity."
    )
        ...
        if results.get("error"):
            ...
            if results.get("abstract_text"):
                ...
                if results.get("abstract_source"):
                    click.echo(
                        f"Source: {results['abstract_source']} ({results.get('abstract_url', '')})"
                    )
                ...
                click.echo(
                    "No specific abstract, definition, or related topics found via DDG Instant Answer."
                )
...
def llm_list_skills():
    ...
    click.echo(
        "Note: LLM-based skills require OPENAI_API_KEY and OPENAI_ORG_ID environment variables to be set."
    )
...
def llm_invoke(plugin_name, function_name, args_json):
    ...
    llm_module = LLMWrapperModule()
    result = asyncio.run(
        llm_module.invoke_skill_async(plugin_name, function_name, parsed_args)
    )
...
@click.argument(
    "cacm_filepath", type=click.Path(exists=True, dir_okay=False, resolve_path=True)
)
@click.option(
    "--output-dir",
    ...
    if results.get("status") == "success" and results.get("outputs"):
        ...
        for out_name, out_data in results.get("outputs", {}).items():
            ...
            if (
                ...
                and "content" in out_val
                and "file_path" in out_val
            ):
                click.echo(
                    f"- {out_name}: Report package (see logs for save path if verbose)"
                )
            else:
                click.echo(f"- {out_name}: (type: {type(out_val).__name__})")
    elif results.get("status") == "failed" and not verbose:
        click.secho(
            "Workflow execution failed. Use --verbose for detailed logs.", fg="yellow"
        )
...
@click.option(
    "--namespace",
    default=None,
    help="Namespace prefix to filter classes (e.g., 'adkarch', 'cacm_ont').",
)
@click.option(
    "--ontology-path",
    type=click.Path(exists=True, dir_okay=False, resolve_path=True),
    default=None,
    help="Custom path to the ontology TTL file.",
)
