...
class DataAccessModule(ToolkitModule):
    ...
    def execute(self, context: dict, db_path: str, query: str, parameters: tuple = None) -> dict:
        """
        ...
        if not query.lstrip().upper().startswith("SELECT"):
            ...
            if not query.lstrip().upper().startswith("PRAGMA"):
                return {"error": "Only SELECT or PRAGMA queries are allowed in this basic version."}
...
if __name__ == "__main__":
    ...
    cursor.execute("INSERT INTO users (name, email) VALUES (?, ?)", ("Alice Wonderland", "alice@example.com"))
    cursor.execute("INSERT INTO users (name, email) VALUES (?, ?)", ("Bob The Builder", "bob@example.com"))
    conn.commit()
    conn.close()

    print("\n--- Test 1: Valid SELECT query ---")
    results1 = data_module.execute(context={}, db_path=TEST_DB_PATH, query="SELECT id, name FROM users WHERE id = ?", parameters=(1,))
    print(json.dumps(results1, indent=2))

    print("\n--- Test 2: Valid SELECT query (all users) ---")
    results2 = data_module.execute(context={}, db_path=TEST_DB_PATH, query="SELECT * FROM users")
    print(json.dumps(results2, indent=2))

    print("\n--- Test 3: PRAGMA query ---")
    results_pragma = data_module.execute(context={}, db_path=TEST_DB_PATH, query="PRAGMA table_info(users);")
    print(json.dumps(results_pragma, indent=2))

    print("\n--- Test 4: Non-SELECT query (should be blocked) ---")
    results3 = data_module.execute(context={}, db_path=TEST_DB_PATH, query="INSERT INTO users (name, email) VALUES ('Charlie', 'charlie@example.com')")
    print(json.dumps(results3, indent=2))
    
    print("\n--- Test 5: Query on non-existent DB ---")
    results4 = data_module.execute(context={}, db_path="non_existent.db", query="SELECT * FROM users")
    ...
    results_syntax_error = data_module.execute(context={}, db_path=TEST_DB_PATH, query="SELECT FROMM users")
