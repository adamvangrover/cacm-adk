...
def get_keywords_and_concepts(config: Dict[str, Any]) -> None:
    ...
    if config["related_keywords"]:
        ...
        for term in config["related_keywords"]:
            definition = get_user_input(
                f"Can you provide a brief definition for '{term}'? (Press Enter to skip)"
            )
            if definition:
...
def determine_information_needs(config: Dict[str, Any]) -> None:
    ...
    if config["information_categories"]:
        ...
        for category in config["information_categories"]:
            ...
            specific_uris = [
                uri.strip() for uri in specific_uris_str.split(",") if uri.strip()
            ]
...
def configure_federated_learning(config: Dict[str, Any]) -> None:
    ...
    if use_fl == "yes":
        ...
        available_categories = [
            ds_input["category_name"]
            for ds_input in config.get("data_sources_input", [])
        ]
        if not available_categories:
            print(
                "Warning: No data source categories were defined in the previous step. Cannot select sources for federated learning."
            )
            config["federated_learning_setup"]["source_categories"] = []
        else:
            print(
                f"Available data source categories for potential FL participation: {', '.join(available_categories)}"
            )
            ...
            if fl_source_categories_str.upper() == "ALL":
                config["federated_learning_setup"][
                    "source_categories"
                ] = available_categories
            else:
                ...
                config["federated_learning_setup"]["source_categories"] = [
                    cat.strip()
                    for cat in fl_source_categories_str.split(",")
                    if cat.strip() in available_categories
                ]
...
def generate_data_artifact_definitions(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for source_input in config.get("data_sources_input", []):
        ...
        first_uri = (
            source_input["specific_uris"][0] if source_input["specific_uris"] else ""
        )

        if (
            "api" in source_input["source_types_description"].lower()
            or "api" in first_uri.lower()
        ):
            data_type = "structured_api_feed"  # Conceptual type
            mime_type = "application/json"
        elif (
            "news" in category_name.lower()
            or "website" in source_input["source_types_description"].lower()
            or first_uri.startswith("http")
        ):
            ...
            "lexicon" in category_name.lower()
            or "word list" in source_input["source_types_description"].lower()
        ):
            ...
            for uri_count, uri in enumerate(source_input["specific_uris"]):
                artifact_def = {
                    ...
                    "metadata": {
                        ...
                        "user_input_source_description": source_input[
                            "source_types_description"
                        ],
...
def generate_ml_model_definition(
    config: Dict[str, Any], data_artifact_ids: List[str]
    ...
    if not learning_task or not learning_task.get("task_type"):
        print(
            "Skipping ML Model definition: No learning task was specified by the user."
        )
        return None
    ...
    input_schema = {
        ...
        "properties": {
            "input_feature_1": {
                "type": "string",
                "description": "Example input feature based on user description.",
            }
        },
    }
    ...
    output_schema = {
        ...
        "properties": {
            "output_value_1": {
                "type": "string",
                "description": "Example output value based on user description.",
            }
        },
    }
    if (
        "label" in learning_task["output_description"].lower()
        or "category" in learning_task["output_description"].lower()
    ):
    ...
    model_def = {
        ...
        "context_dependencies": config.get(
            "related_keywords", []
        ),  # Keywords can be context dependencies
...
def generate_federated_learning_setup(
    config: Dict[str, Any],
    ...
    for category in fl_setup_input.get("source_categories", []):
        if (
            category in artifact_category_to_id_map
            and artifact_category_to_id_map[category]
        ):
            participating_artifact_ids_by_category[category] = (
                artifact_category_to_id_map[category]
            )
        else:
            print(
                f"Warning (FL Setup): Category '{category}' selected for FL, but no corresponding DataArtifacts found. It will be excluded."
            )

    if not participating_artifact_ids_by_category:
        print(
            "Warning (FL Setup): No valid DataArtifacts found for any selected FL categories. Skipping FL setup generation."
        )
        return None

    num_potential_nodes = len(
        participating_artifact_ids_by_category
    )  # Assuming one node per category for this conceptual setup.
...
def build_expertise_interactively():
    ...
    expertise_config: Dict[str, Any] = {
        ...
        "expertise_id": str(
            uuid.uuid4()
        ),  # Unique ID for this entire expertise definition session.
    }
    ...
    configure_federated_learning(
        expertise_config
    )  # Depends on data_sources_input being populated
    ...
    if artifact_defs:
        ...
        print(
            f"  - The '{artifact_filename}' contains definitions for potential DataArtifacts."
        )
        print(
            f"    These can be used to register artifacts in a KnowledgeStore instance from framework.py."
        )
        print(
            f"    Each definition includes a 'source_uri', 'data_type', and initial 'status'."
        )
    else:
    ...
    model_id_for_fl: Optional[str] = (
        None  # To store the model ID if a model is defined.
    )
    ...
    if model_def:
        ...
        # Guidance for the user:
        print(
            f"  - The '{model_filename}' contains a definition for a proposed MLModel."
        )
        print(
            f"    This can be used to register a model with MachineLearningGuidance from framework.py."
        )
        print(
            f"    It includes suggested 'model_type', I/O schemas, and links to 'training_data_artifacts'."
        )

    else:
        print(
            "  - No ML model definition was generated based on your input (e.g., learning task not specified)."
        )
    ...
    if model_id_for_fl:  # Only attempt to generate FL setup if an ML model was defined.
        fl_setup_def = generate_federated_learning_setup(
            expertise_config, model_id_for_fl, artifact_category_to_id_map
        )
        if fl_setup_def:
            ...
            # Guidance for the user:
            print(
                f"  - The '{fl_filename}' contains a definition for a Federated Learning setup."
            )
            print(
                f"    This can be used to configure a FederatedLearningInfra instance from framework.py."
            )
            print(
                f"    It links to the global model and maps data categories to artifact IDs for FL participation."
            )
        else:
            if expertise_config.get("federated_learning_setup", {}).get("enabled"):
                print(
                    "  - Federated learning was enabled, but its setup definition could not be fully generated (e.g., no suitable data artifacts found for selected categories)."
                )
            else:
                print(
                    "  - Federated learning was not enabled by the user; no FL setup file generated."
                )
    else:
        print(
            "  - Skipping federated learning setup generation as no ML model was defined."
        )
    ...
    print(
        "You can now use the generated JSON files to populate your data mesh ontology framework."
    )
