...
SECTION_MARKERS = [
    ...
    ),  # New addition based on recent SEC rules
    ...
    (
        "ITEM_5_MARKET_FOR_REGISTRANT_COMMON_EQUITY",
    ...
    (
        "ITEM_7A_QUANTITATIVE_QUALITATIVE_MARKET_RISK",
        [
    ...
    (
        "ITEM_9_CHANGES_DISAGREEMENTS_ACCOUNTANTS",
        [r"ITEM\s+9\.\s*CHANGES\s*IN\s*AND\s*DISAGREEMENTS\s*WITH\s*ACCOUNTANTS"],
    ),
    ...
    (
        "ITEM_9C_DISCLOSURE_FOREIGN_JURISDICTIONS_INSPECTIONS",
        [r"ITEM\s+9C\.\s*DISCLOSURE\s*REGARDING\s*FOREIGN\s*JURISDICTIONS"],
    ),  # Simplified
    (
        "PART_II_OTHER_INFORMATION_GENERAL",
        [r"PART\s+II", r"PART\s+II\."],
    ),  # More generic PART II start, if specific items missed.
    ...
    (
        ...
        [
            r"ITEM\s+15\.\s*EXHIBITS,\s*FINANCIAL\s*STATEMENT\s*SCHEDULES",
            r"ITEM\s+15\.\s*EXHIBIT\s*AND\s*FINANCIAL\s*STATEMENT\s*SCHEDULES",
        ],
    ),
...
def extract_sections_from_text_filing(document_path: str) -> dict:
    ...
    for key, patterns in SECTION_MARKERS:
        for pattern in patterns:
            try:
                ...
                print(
                    f"Warning: Regex error for pattern '{pattern}' on key '{key}': {e}"
                )
    ...
    for i, marker_info in enumerate(found_marker_positions):
        ...
        text_start_position = marker_info[
            "end"
        ]  # Start extracting text *after* the marker itself
        ...
        ):  # Add only if key not already populated (first occurrence wins)
            sections[section_key] = section_content
        elif (
            not sections[section_key] and section_content
        ):  # If previous was empty, fill it
            sections[section_key] = section_content
        ...
        if len(text) < 100 and text.isupper() and key != "__UNMATCHED_PREAMBLE__":
            ...
            for _, patterns in SECTION_MARKERS:
                ...
                if is_another_marker:
                    break
            if (
                not is_another_marker and len(text.split()) < 15
            ):  # if less than 15 words and uppercase
                print(
                    f"Info: Potentially removing short/header-like section '{key}' with content: '{text[:50]}...'"
                )
...
if __name__ == "__main__":
    ...
    if "error" not in sectioned_data:
        try:
            ...
            if sectioned_data:
                actual_sections_empty = all(
                    not text
                    for key, text in sectioned_data.items()
                    if key != "__UNMATCHED_PREAMBLE__"
                )

            if not sectioned_data or actual_sections_empty:
                print(
                    "Warning: The output is empty or contains mostly preamble/empty actual sections. Check input file content and section markers."
                )
                if (
                    "__UNMATCHED_PREAMBLE__" in sectioned_data
                    and len(sectioned_data) == 1
                ):
                    print("Info: Only preamble was found.")
            elif (
                len(sectioned_data) <= 2
                and "__UNMATCHED_PREAMBLE__" in sectioned_data
                and not actual_sections_empty
            ):  # Preamble + one actual section
                print(
                    f"Info: Few sections extracted. Found: {list(sectioned_data.keys())}."
                )
            elif (
                len(sectioned_data) <= 1
                and not "__UNMATCHED_PREAMBLE__" in sectioned_data
                and not actual_sections_empty
            ):  # Only one actual section
                print(
                    f"Info: Only one actual section extracted. Found: {list(sectioned_data.keys())}."
                )
    ...
    if "error" not in sectioned_data_10q:
        try:
            ...
            if sectioned_data_10q:
                actual_sections_10q_empty = all(
                    not text
                    for key, text in sectioned_data_10q.items()
                    if key != "__UNMATCHED_PREAMBLE__"
                )

            if not sectioned_data_10q or actual_sections_10q_empty:
                print(
                    "Warning: The 10-Q output is empty or contains mostly preamble/empty actual sections. Check input file content and section markers."
                )
        except Exception as e:
