...
def calculate_ratio(numerator: Union[int, float], denominator: Union[int, float]) -> Optional[float]:
    ...
    if not isinstance(numerator, (int, float)) or not isinstance(denominator, (int, float)):
        ...
        print(f"ERROR: calculate_ratio: Invalid input types: num={type(numerator)}, den={type(denominator)}")
        return None
    if denominator == 0:
...
def simple_scorer(financial_metric: Union[int, float], threshold: Union[int, float], operator: str = '>') -> str:
    """
    ...
    Supported operators: '>', '<', '>=', '<=', '==', '!='.
    """
    if not isinstance(financial_metric, (int, float)) or not isinstance(threshold, (int, float)):
        print(f"ERROR: simple_scorer: Invalid input types: metric={type(financial_metric)}, threshold={type(threshold)}")
