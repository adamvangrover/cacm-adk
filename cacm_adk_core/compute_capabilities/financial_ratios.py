...
def calculate_basic_ratios(financial_data: dict, rounding_precision: int = 2) -> dict:
    ...
    for key in required_keys:
        if key not in financial_data:
            errors.append(
                f"Missing required financial data key: {key} (for {required_keys[key]})"
            )
            missing_keys = True
    ...
    if current_liabilities == 0:
        ...
        try:
            calculated_ratios_intermediate["current_ratio"] = round(
                current_assets / current_liabilities, rounding_precision
            )
        except TypeError:  # Should be caught by earlier checks, but as a safeguard
            errors.append(
                "Type error during Current Ratio calculation. Ensure inputs are numeric."
            )
    ...
    if total_equity == 0:
        ...
        try:
            calculated_ratios_intermediate["debt_to_equity_ratio"] = round(
                total_debt / total_equity, rounding_precision
            )
        except TypeError:  # Safeguard
            errors.append(
                "Type error during Debt-to-Equity Ratio calculation. Ensure inputs are numeric."
            )
        except Exception as e:  # Catch any other unexpected calculation error
            errors.append(
                f"Unexpected error calculating Debt-to-Equity Ratio: {str(e)}"
            )
