...
def fetch_company_profile_mock(company_identifier: str) -> Dict[str, Any]:
    ...
    if company_identifier in MOCKED_PROFILES:
        return MOCKED_PROFILES[company_identifier].copy() # Return a copy to prevent modification of mock data
    else:
        return {
            "companyName": None,
...
if __name__ == '__main__':
    ...
    if returned_profile_copy.get("keyExecutives"):
        ...
        profile_to_modify_for_test["keyExecutives"].append({"name": "Test User", "title": "Tester"})
        print("Locally modified deepcopy of COMP_ABC's data:")
        print(json.dumps(profile_to_modify_for_test, indent=2))

    print("Original MOCKED_PROFILES['COMP_ABC'] (should be unchanged by the deepcopy modification):")
