...
from cacm_adk_core.semantic_kernel_adapter import (
    KernelService,
)  # For consistency, though not directly used by this agent's current logic
...
class ReportGenerationAgent(Agent):
    ...
    def __init__(self, kernel_service: KernelService):
        super().__init__(
            agent_name="ReportGenerationAgent", kernel_service=kernel_service
        )
    ...
    async def run(
        ...
        fundamental_analysis_data = current_step_inputs.get(
            "fundamental_analysis_data_ref"
        )
        snc_analysis_data = current_step_inputs.get("snc_analysis_data_ref")
        catalyst_data = current_step_inputs.get("catalyst_data_ref")
        output_dir = current_step_inputs.get(
            "output_dir", "./output_artifacts/default_report_output"
        )  # Get output_dir
        ...
        company_name = shared_context.get_data(
            "company_name", "N/A"
        )  # Still useful for title
        company_ticker = shared_context.get_data(
            "company_ticker", "N/A"
        )  # Still useful
        business_overview_text = shared_context.get_data(
            "structured_financials_for_summary", "[Business Overview Not Available]"
        )
        risk_factors_text = shared_context.get_data(
            "risk_factors_section_text", "[Risk Factors Text Not Available]"
        )

        self.logger.info(
            f"Retrieved data for report generation. Company: {company_name}"
        )

        report_parts = []

        report_title_detail = current_step_inputs.get(
            "report_title_detail", "Comprehensive Analysis Report"
        )
        report_parts.append(
            f"# {report_title_detail} for {company_name} ({company_ticker})"
        )
        report_parts.append(f"## Task: {task_description}")
        report_parts.append(
            f"Generated by: {self.agent_name} for CACM ID: {shared_context.get_cacm_id()} (Session: {shared_context.get_session_id()})"
        )
        report_parts.append("---")
        ...
        if fundamental_analysis_data and isinstance(
            fundamental_analysis_data, dict
        ):  # Input is already the 'data' payload
            ...
            report_parts.append(
                f"\n### DCF Valuation: ${faa_data.get('dcf_valuation', 'N/A'):,.2f}"
                if isinstance(faa_data.get("dcf_valuation"), (int, float))
                else f"\n### DCF Valuation: {faa_data.get('dcf_valuation', 'N/A')}"
            )
            report_parts.append(
                f"### Enterprise Value: ${faa_data.get('enterprise_value', 'N/A'):,.2f}"
                if isinstance(faa_data.get("enterprise_value"), (int, float))
                else f"### Enterprise Value: {faa_data.get('enterprise_value', 'N/A')}"
            )
            report_parts.append(
                f"### Financial Health Assessment: {faa_data.get('financial_health', 'N/A')}"
            )
            report_parts.append(
                "\n### Analysis Summary (from FundamentalAnalystAgent, incorporates SK-processed Press Release Insights):"
            )
            report_parts.append(
                faa_data.get("analysis_summary", "[Summary not available]")
            )
            ...
            if press_release_summaries_faa:
                ...
                for key, summary in press_release_summaries_faa.items():
                    ...
                    display_key = (
                        key.replace("summary_press_release_", "")
                        .replace("_", " ")
                        .upper()
                    )
            ...
            report_parts.append(
                "Fundamental analysis data not available or was not successfully processed by the upstream agent."
            )
        ...
        if snc_analysis_data and isinstance(
            snc_analysis_data, dict
        ):  # Input is already the 'data' payload
            sncaa_data = snc_analysis_data
            report_parts.append(
                f"**SNC Rating:** {sncaa_data.get('rating', '[Rating not provided]')}"
            )
            report_parts.append(
            ...
            press_release_insights_snc = sncaa_data.get(
                "sk_generated_press_release_insights", {}
            )
            if press_release_insights_snc:
                ...
                for key, insight in press_release_insights_snc.items():
                    ...
                    display_key = (
                        key.replace("insights_press_release_", "")
                        .replace("_", " ")
                        .upper()
                    )
            ...
            report_parts.append(
                "SNC analysis data not available or was not successfully processed by the upstream agent."
            )
            # No need to print snc_analysis_data.get('message')

        # Catalyst Strategic Insights Section
        report_parts.append("\n## 4. Catalyst Strategic Insights")
        if catalyst_data and isinstance(
            catalyst_data, dict
        ):  # Input is already the 'data' payload
            ...
            report_parts.append(
                "Catalyst strategic insights not available or was not successfully processed by the upstream agent."
            )
        ...
        if current_step_inputs:
            ...
            filtered_inputs_for_display = {
                k: v
                for k, v in current_step_inputs.items()
                if not k.endswith("_data_ref")
            }
            report_parts.append(
        ...
        if hasattr(self, "stored_results") and self.stored_results:
            ...
            for i, res_item in enumerate(self.stored_results):
                report_parts.append(
                    f"**Item {i+1} from Agent '{res_item.get('from', 'Unknown')}':**"
                )
                report_parts.append(
        ...
        report_file_path = (
            f"{output_dir.rstrip('/')}/{report_filename}"  # Use the provided output_dir
        )

        self.logger.info(
            f"Report generation complete. Conceptual save path: {report_file_path}"
        )
        self.logger.debug(
            f"Report Content for {report_filename}:\n{final_report_string}"
        )
    ...
    async def receive_analysis_results(
        self, sending_agent_name: str, results: Dict[str, Any]
    ):
        """
        ...
        """
        self.logger.info(
            f"'{self.agent_name}' received analysis results from '{sending_agent_name}'. Storing under 'stored_results'."
        )
        if not hasattr(self, "stored_results"):
            self.stored_results = []
        self.stored_results.append({"from": sending_agent_name, "data": results})
        self.logger.info(
            f"Results from '{sending_agent_name}' stored. Total stored items: {len(self.stored_results)}"
        )
...
if __name__ == "__main__":
    ...
    class MockKernelService(KernelService):  # Minimal mock
        def __init__(self):
            self.logger = logging.getLogger("MockReportGenKernelService")
    ...
    test_shared_context.set_data(
        "financial_performance_summary_text",
        "[Placeholder Financial Summary - Test Data]",
    )
    test_shared_context.set_data(
        "key_risks_summary_text", "[Placeholder Risk Summary - Test Data]"
    )
    test_shared_context.set_data(
        "overall_assessment_text", "[Placeholder Overall Assessment - Test Data]"
    )
