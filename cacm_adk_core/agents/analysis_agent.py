...
class AnalysisAgent(Agent):
    def __init__(self, kernel_service: KernelService):
        super().__init__(
            ...
            skills_plugin_name="FinancialAnalysis",
        )
    ...
    async def run(
        ...
        final_message = (
            "Analysis phase completed successfully."  # Default success message
        )

        financial_summary_text = "[Financial Performance Summary Not Available - Generation Skipped or Failed]"
        key_risks_summary_text = (
            "[Key Risks Summary Not Available - Generation Skipped or Failed]"
        )
        overall_assessment_text = (
            "[Overall Assessment Not Available - Generation Skipped or Failed]"
        )
        ratios_skill_payload = None

        expanded_financial_data = shared_context.get_data(
            "financial_data_for_ratios_expanded"
        )
        structured_financials_for_summary = shared_context.get_data(
            "structured_financials_for_summary"
        )
        ...
        if not kernel:
            ...
            if not expanded_financial_data:
                agent_ops_summary["ratio_calculation"] = (
                    "Skipped: Missing 'financial_data_for_ratios_expanded'."
                )
                if final_status == "success":
                    final_status = "warning"
            elif not self.skills_plugin_name:
                agent_ops_summary["ratio_calculation"] = (
                    "Skipped: No FinancialAnalysis plugin configured."
                )
                if final_status == "success":
                    final_status = "warning"
            else:
                ...
                if not fin_plugin:
                    agent_ops_summary["ratio_calculation"] = (
                        f"Error: Plugin '{self.skills_plugin_name}' not found."
                    )
                    final_status = "error"
                    final_message = agent_ops_summary["ratio_calculation"]
                else:
                    ratio_func = fin_plugin.get("calculate_basic_ratios")
                    if not ratio_func:
                        agent_ops_summary["ratio_calculation"] = (
                            "Error: 'calculate_basic_ratios' function not found."
                        )
                        final_status = "error"
                        final_message = agent_ops_summary["ratio_calculation"]
                    else:
                        try:
                            args = KernelArguments(
                                financial_data=expanded_financial_data,
                                rounding_precision=current_step_inputs.get(
                            ...
                            shared_context.set_data(
                                "calculated_key_ratios",
                                ratios_skill_payload.get("calculated_ratios", {}),
                            )
                            agent_ops_summary["ratio_calculation"] = (
                                f"Success: {ratios_skill_payload.get('calculated_ratios', {})}"
                            )
                            if ratios_skill_payload.get("errors"):
                                final_status = "partial_success"
                        except Exception as e:
                            agent_ops_summary["ratio_calculation"] = (
                                f"Error invoking 'calculate_basic_ratios' skill: {e}"
                            )
                            final_status = "error"
                            final_message = agent_ops_summary["ratio_calculation"]
            ...
            if final_status not in ["error"] and expanded_financial_data:
                ...
                historical_stats = {
                    "current_assets_mean": 600000.0,
                    "revenue_mean": 2200000.0,
                    "total_debt_mean": 400000.0,
                }
                ...
                for key, hist_mean in historical_stats.items():
                    ...
                    if current_value is not None:
                        try:
                            ...
                            deviation = (
                                abs(current_value_float - hist_mean) / hist_mean
                                if hist_mean != 0
                                else float("inf")
                            )
                            ...
                            self.logger.warning(
                                f"Could not convert current value of '{key}' ({current_value}) to float for data drift check."
                            )
                    else:
                        self.logger.info(
                            f"Key '{key}' not present in current expanded_financial_data for data drift check."
                        )

                if data_drift_detected_messages:
                    agent_ops_summary["conceptual_data_drift_check"] = (
                        "Potential data drift detected: "
                        + "; ".join(data_drift_detected_messages)
                    )
                    if final_status == "success":
                        final_status = "warning"
                    if (
                        "Potential data drift detected" not in final_message
                        and "Analysis phase completed successfully." in final_message
                    ):
                        final_message = "Analysis completed with potential data drift detected (conceptual check)."
                    elif "Potential data drift detected" not in final_message:
                        final_message += (
                            " Potential data drift detected (conceptual check)."
                        )

                else:
                    agent_ops_summary["conceptual_data_drift_check"] = (
                        "No significant data drift detected (placeholder logic)."
                    )
            elif not expanded_financial_data:
                agent_ops_summary["conceptual_data_drift_check"] = (
                    "Skipped: expanded_financial_data not available."
                )

            # Conceptual Model Drift Check (on a calculated ratio)
            if final_status not in ["error"] and ratios_skill_payload:
                self.logger.info(
                    "Performing conceptual model drift check on current_ratio."
                )
                calculated_ratios_map = ratios_skill_payload.get(
                    "calculated_ratios", {}
                )
                ...
                if current_ratio_value is not None:
                    try:
                        ...
                        if (
                            abs(current_ratio_float - historical_avg_current_ratio)
                            ...
                            if final_status == "success":
                                final_status = "warning"
                            if (
                                "Potential model drift detected" not in final_message
                                and "Analysis phase completed successfully."
                                in final_message
                            ):
                                final_message = "Analysis completed with potential model drift detected (conceptual check)."
                            elif "Potential model drift detected" not in final_message:
                                final_message += " Potential model drift detected (conceptual check)."
                        else:
                            agent_ops_summary["conceptual_model_drift_check"] = (
                                f"No significant model output drift detected for current_ratio ({current_ratio_float:.2f}) vs historical ({historical_avg_current_ratio:.2f}) (placeholder logic)."
                            )
                    except ValueError:
                        self.logger.warning(
                            f"Could not convert current_ratio value ('{current_ratio_value}') to float for model drift check."
                        )
                        agent_ops_summary["conceptual_model_drift_check"] = (
                            "Skipped: current_ratio value not numeric."
                        )
                else:
                    agent_ops_summary["conceptual_model_drift_check"] = (
                        "Skipped: 'current_ratio' not found in calculated ratios."
                    )
            elif not ratios_skill_payload:
                agent_ops_summary["conceptual_model_drift_check"] = (
                    "Skipped: Ratio calculation payload not available."
                )

            # Summarization Skills (only if no critical error yet)
            if final_status not in [
                "error"
            ]:  # This check might be redundant if already error, but safe
                ...
                if report_plugin:
                    ...
                    if fin_summary_fn:
                        if structured_financials_for_summary:
                            try:
                                self.logger.info(
                                    "Invoking 'generate_financial_summary' LLM skill."
                                )
                                result = await kernel.invoke(
                                    fin_summary_fn,
                                    KernelArguments(
                                        financial_data=structured_financials_for_summary
                                    ),
                                )
                                ...
                                shared_context.set_data(
                                    "financial_performance_summary_llm",
                                    financial_summary_text,
                                )
                                if (
                                    "[Placeholder" in financial_summary_text
                                    or "[LLM" in financial_summary_text
                                ):
                                    agent_ops_summary[
                                        "financial_summary_generation"
                                    ] = f"Success (LLM returned placeholder/error: {financial_summary_text[:100]})"
                                    if final_status == "success":
                                        final_status = "partial_success"
                                else:
                                    agent_ops_summary[
                                ...
                                agent_ops_summary["financial_summary_generation"] = (
                                    f"Error invoking skill: {e}"
                                )
                                final_status = "error"
                                final_message = (
                                    f"Error in financial summary generation: {e}"
                                )
                        else:
                            agent_ops_summary["financial_summary_generation"] = (
                                "Skipped: No 'structured_financials_for_summary' in shared_context."
                            )
                        ...
                        agent_ops_summary["financial_summary_generation"] = (
                            "Skipped: 'generate_financial_summary' function not found in plugin."
                        )
                        financial_summary_text = "[Financial Performance Summary Not Available - Skill Missing]"
                        shared_context.set_data(
                            "financial_performance_summary_llm", financial_summary_text
                        )
                        if final_status == "success":
                            final_status = "warning"
                    ...
                    if key_risks_fn:
                        if risk_factors_section_text:
                            try:
                                self.logger.info(
                                    "Invoking 'generate_key_risks_summary' LLM skill."
                                )
                                result = await kernel.invoke(
                                    key_risks_fn,
                                    KernelArguments(
                                        risk_factors_text=risk_factors_section_text
                                    ),
                                )
                                ...
                                shared_context.set_data(
                                    "key_risks_summary_llm", key_risks_summary_text
                                )
                                if (
                                    "[Placeholder" in key_risks_summary_text
                                    or "[LLM" in key_risks_summary_text
                                ):
                                    agent_ops_summary["risk_summary_generation"] = (
                                        f"Success (LLM returned placeholder/error: {key_risks_summary_text[:100]})"
                                    )
                                    if final_status == "success":
                                        final_status = "partial_success"
                                else:
                                ...
                                final_status = "error"
                                final_message = f"Error in risk summary generation: {e}"
                        else:
                            agent_ops_summary["risk_summary_generation"] = (
                                "Skipped: No 'risk_factors_section_text' in shared_context."
                            )
                            key_risks_summary_text = (
                                "[Key Risks Summary Not Available - Input Missing]"
                            )
                            shared_context.set_data(
                                "key_risks_summary_llm", key_risks_summary_text
                            )
                        ...
                        agent_ops_summary["risk_summary_generation"] = (
                            "Skipped: 'generate_key_risks_summary' function not found in plugin."
                        )
                        key_risks_summary_text = (
                            "[Key Risks Summary Not Available - Skill Missing]"
                        )
                        shared_context.set_data(
                            "key_risks_summary_llm", key_risks_summary_text
                        )
                    ...
                    overall_assessment_fn = report_plugin.get(
                        "generate_overall_assessment"
                    )
                    if overall_assessment_fn:
                        calculated_ratios_map = shared_context.get_data(
                            "calculated_key_ratios", {}
                        )
                        ...
                        current_financial_summary = shared_context.get_data(
                            "financial_performance_summary_llm", financial_summary_text
                        )
                        current_key_risks_summary = shared_context.get_data(
                            "key_risks_summary_llm", key_risks_summary_text
                        )
                        try:
                            self.logger.info(
                                "Invoking 'generate_overall_assessment' LLM skill."
                            )
                            args = KernelArguments(
                            ...
                            shared_context.set_data(
                                "overall_assessment_llm", overall_assessment_text
                            )
                            if (
                                "[Placeholder" in overall_assessment_text
                                or "[LLM" in overall_assessment_text
                            ):
                                agent_ops_summary["overall_assessment_generation"] = (
                                    f"Success (LLM returned placeholder/error: {overall_assessment_text[:100]})"
                                )
                                if final_status == "success":
                                    final_status = "partial_success"
                            else:
                            ...
                            overall_assessment_text = (
                                "[LLM Overall Assessment generation failed in agent.]"
                            )
                            shared_context.set_data(
                                "overall_assessment_llm", overall_assessment_text
                            )
                            ...
                            final_status = "error"
                            final_message = (
                                f"Error in overall assessment generation: {e}"
                            )
                    else:
                        agent_ops_summary["overall_assessment_generation"] = (
                            "Skipped: 'generate_overall_assessment' function not found in plugin."
                        )
                        overall_assessment_text = (
                            "[Overall Assessment Not Available - Skill Missing]"
                        )
                        shared_context.set_data(
                            "overall_assessment_llm", overall_assessment_text
                        )
                    ...
                    if any(
                        ...
                        if final_status == "success":
                            final_status = "partial_success"  # Already partial_success if placeholder
                        if final_message == "Analysis phase completed successfully.":
                    ...
                    agent_ops_summary["financial_summary_generation"] = (
                        ...
                        "overall_assessment_generation"
                    ] = "Skipped: ReportingAnalysisSkills plugin not found."
                    shared_context.set_data(
                        "financial_performance_summary_llm", financial_summary_text
                    ...
                    if final_status == "success":
                        final_status = "warning"
                        final_message = "Analysis completed, but ReportingAnalysisSkills plugin for LLM summaries was not found."

            # Ratio Explanation Generation (after ratio calculation and summarization, if plugin available)
            if (
                final_status not in ["error"] and report_plugin
            ):  # report_plugin check implies kernel is available
                ...
                if calculated_ratios:
                    ...
                    if explanation_skill_func:
                        self.logger.info(
                            f"Found 'generate_explanation' skill. Attempting to explain {len(calculated_ratios)} ratios."
                        )
                        ratio_contexts = {
                        ...
                        for ratio_key, ratio_value in calculated_ratios.items():
                            if ratio_value is None or str(
                                ratio_value
                            ).strip().lower() in ["n/a", "nan", "inf", "-inf", ""]:
                                explanation_text = f"Value for {ratio_key.replace('_', ' ').title()} is not suitable for explanation."
                                key_ratios_explanations_llm[ratio_key] = (
                                    explanation_text
                                )
                            ...
                            data_point_value_str = (
                                f"{ratio_value:.2f}"
                                if isinstance(ratio_value, (float, int))
                                else str(ratio_value)
                            )
                            context_desc = ratio_contexts.get(
                                ratio_key,
                                "A financial ratio indicating company performance or financial health.",
                            )

                            try:
                                self.logger.debug(
                                    f"Invoking 'generate_explanation' for {data_point_name} ({data_point_value_str})."
                                )
                                ...
                                result = await kernel.invoke(
                                    explanation_skill_func, exp_args
                                )
                                ...
                                if (
                                    "[Placeholder" in explanation_text
                                    or "[LLM" in explanation_text
                                    or not explanation_text
                                ):
                                    self.logger.warning(
                                        f"LLM returned placeholder or empty explanation for {data_point_name}."
                                    )
                                    explanation_text = f"LLM explanation for {data_point_name} (value: {data_point_value_str}) was a placeholder or empty."
                                    explanations_failed_or_placeholder += 1
                                else:
                                    self.logger.info(
                                        f"LLM explanation generated for {data_point_name}."
                                    )
                                ...
                                self.logger.error(
                                    f"Error invoking 'generate_explanation' for {data_point_name}: {e}"
                                )
                                key_ratios_explanations_llm[ratio_key] = (
                                    f"Error generating explanation for {data_point_name}: {e}"
                                )
                                explanations_failed_or_placeholder += 1

                        agent_ops_summary["ratio_explanations_generation"] = (
                            f"Success for {explanations_succeeded} ratio(s), Failed/Placeholder for {explanations_failed_or_placeholder} ratio(s)."
                        )
                        if (
                            explanations_failed_or_placeholder > 0
                        ...
                        agent_ops_summary["ratio_explanations_generation"] = (
                            "Skipped: 'generate_explanation' function not found in ReportingAnalysisSkills plugin."
                        )
                        for ratio_key in calculated_ratios.keys():
                            key_ratios_explanations_llm[ratio_key] = (
                                "Explanation generation skill not available."
                            )
                        if final_status == "success" and calculated_ratios:
                            final_status = "warning"
                else:  # No calculated ratios
                    agent_ops_summary["ratio_explanations_generation"] = (
                        "Skipped: No ratios were calculated or available."
                    )

                shared_context.set_data(
                    "key_ratios_explanations_llm", key_ratios_explanations_llm
                )
            ...
            if final_status not in [
                "error"
            ]:  # Proceed only if no critical errors so far
                self.logger.info(
                    "Starting ESG data retrieval and summarization process."
                )
                company_ticker_for_uri = shared_context.get_data(
                    "companyTicker", "unknown_company_ticker"
                )
                # Assuming a base URI structure, this should align with KG population
                company_uri_base_for_query = current_step_inputs.get(
                    "company_uri_base", "http://example.com/entity/"
                )
                company_uri_for_query = (
                    f"{company_uri_base_for_query.rstrip('/')}/{company_ticker_for_uri}"
                )
                ...
                """
                self.logger.debug(
                    f"Formulated SPARQL query for ESG data:\n{esg_sparql_query}"
                )

                try:
                    kg_agent = await self.get_or_create_agent("KnowledgeGraphAgent", {})
                    if not kg_agent:
                        agent_ops_summary["esg_data_processing"] = (
                            "Failed: Could not get KnowledgeGraphAgent."
                        )
                        if final_status == "success":
                            final_status = "error"
                    else:
                        ...
                        self.logger.info(
                            "Invoking KnowledgeGraphAgent to fetch ESG data."
                        )
                        kg_results_payload = await kg_agent.run(
                            "Fetch ESG data from KG", kg_agent_inputs, shared_context
                        )

                        if kg_results_payload.get("status") == "success":
                            actual_kg_results = kg_results_payload.get("data", {}).get(
                                "results", []
                            )
                            agent_ops_summary["esg_data_processing"] = (
                                f"KG query successful, {len(actual_kg_results)} results."
                            )
                            ...
                            if actual_kg_results:
                                ...
                                if not esg_analysis_plugin:
                                    agent_ops_summary[
                                        "esg_data_processing"
                                    ] += " Error: ESGAnalysis plugin not found."
                                    if final_status == "success":
                                        final_status = "error"
                                else:
                                    esg_skill_func = esg_analysis_plugin.get(
                                        "summarize_esg_factors_from_kg"
                                    )
                                    if not esg_skill_func:
                                        agent_ops_summary[
                                            "esg_data_processing"
                                        ] += " Error: summarize_esg_factors_from_kg function not found."
                                        if final_status == "success":
                                            final_status = "error"
                                    else:
                                        company_name_for_summary = (
                                            shared_context.get_data(
                                                "companyName", "N/A"
                                            )
                                        )
                                        esg_args = KernelArguments(
                                            kg_query_results=actual_kg_results,
                                            company_name=company_name_for_summary,
                                        )

                                        self.logger.info(
                                            f"Invoking ESGAnalysisSkill to summarize {len(actual_kg_results)} ESG factors."
                                        )
                                        summary_result = await kernel.invoke(
                                            esg_skill_func, esg_args
                                        )
                                        summarized_esg_data = summary_result.value

                                        shared_context.set_data(
                                            "summarized_esg_data", summarized_esg_data
                                        )  # Changed key from _llm
                                        agent_ops_summary[
                                            "esg_data_processing"
                                        ] += " Summarization successful."
                                        self.logger.info(
                                            f"ESG data summarized and stored in SharedContext: {summarized_esg_data}"
                                        )
                            else:  # No results from KG
                                agent_ops_summary[
                                    "esg_data_processing"
                                ] += " No specific ESG data points found in KG for company."
                                shared_context.set_data(
                                    "summarized_esg_data",
                                    {
                                        "company_name": shared_context.get_data(
                                            "companyName", "N/A"
                                        ),
                                        "notes": "No ESG data points found in KG.",
                                    },
                                )
                                if final_status == "success":
                                    final_status = "partial_success"  # It's not an error, but data is missing
                        else:  # KG Agent failed
                            agent_ops_summary["esg_data_processing"] = (
                                f"Failed: KnowledgeGraphAgent error - {kg_results_payload.get('message', 'Unknown error')}"
                            )
                            if final_status == "success":
                                final_status = "error"
            ...
            if (
                final_status == "success"
                and final_message == "Analysis phase completed successfully."
            ):
                final_message = (
                    "All analysis steps, including ESG processing, completed."
                )
            elif (
                final_status == "partial_success"
                and final_message == "Analysis phase completed successfully."
            ):  # Overwrite default if partial due to ESG
                final_message = agent_ops_summary.get(
                    "esg_data_processing",
                    "Analysis partially completed; check ESG processing status.",
                )
            elif (
                final_status == "error"
                and final_message == "Analysis phase completed successfully."
            ):  # Overwrite default if error due to ESG
                final_message = agent_ops_summary.get(
                    "esg_data_processing",
                    "Analysis error; check ESG processing status.",
                )

        agent_ops_summary["final_message_before_reporting"] = final_message

        # Communication with ReportGenerationAgent
        report_agent = await self.get_or_create_agent(
            "ReportGenerationAgent", {"triggering_agent": self.agent_name}
        )
        if report_agent:
            data_for_report = {
                ...
                "financial_summary": shared_context.get_data(
                    "financial_performance_summary_llm"
                ),
            ...
            try:
                await report_agent.receive_analysis_results(
                    self.agent_name, data_for_report
                )
                agent_ops_summary["reporting_to_other_agent"] = "Success."
            except Exception as e:
                agent_ops_summary["reporting_to_other_agent"] = f"Error: {e}"
                final_status = "error"
                final_message = f"Failed sending results to ReportGenerationAgent: {e}"
        else:
            agent_ops_summary["reporting_to_other_agent"] = (
                "Failed: Could not get or create ReportGenerationAgent."
            )
            final_status = "error"
            final_message = "Critical error: Failed to obtain ReportGenerationAgent."
        ...
        return {
            ...
            "text_summaries_generated": {
                "financial_performance": shared_context.get_data(
                    "financial_performance_summary_llm"
                ),
