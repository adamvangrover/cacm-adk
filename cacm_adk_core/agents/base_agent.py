...
class Agent(ABC):
    ...
    """
    def __init__(self, agent_name: str, kernel_service: KernelService, skills_plugin_name: Optional[str] = None):
        """
        ...
        self.logger = logging.getLogger(f"agent.{self.agent_name}") # Per-instance logger
        self.logger.info(f"Agent '{self.agent_name}' created. Associated skill plugin: {self.skills_plugin_name if self.skills_plugin_name else 'N/A'}")
    ...
    async def get_or_create_agent(self, agent_name_key: str, context_data: Optional[Dict[str, any]] = None) -> Optional['Agent']:
        """
        Requests an instance of another agent from the agent manager.
        """
        self.logger.info(f"'{self.agent_name}' is requesting agent '{agent_name_key}' with context: {context_data if context_data else 'None'}")
        if not self.agent_manager:
            self.logger.error("Agent manager not set. Cannot get or create other agents.")
            return None
        ...
        try:
            ...
            agent_instance = await self.agent_manager.get_or_create_agent_instance(
                agent_name_key,
                context_data_for_creation=context_data_for_creation
            )
            if agent_instance:
                self.logger.info(f"Successfully obtained instance of '{agent_name_key}'.")
            else:
                self.logger.warning(f"Failed to obtain instance of '{agent_name_key}'.")
            return agent_instance
        except Exception as e:
            self.logger.error(f"Error while trying to get/create agent '{agent_name_key}': {e}", exc_info=True)
            return None
    ...
    def get_plugin(self, plugin_name: str): # -> Optional[KernelPlugin]: Type hint KernelPlugin if available
        """
        ...
        if kernel and kernel.plugins:
            try:
                ...
                logger.warning(f"Plugin '{plugin_name}' not found in kernel for agent '{self.agent_name}'.")
                return None
        logger.warning(f"Kernel or plugins collection not available for agent '{self.agent_name}'.")
        return None
...
if __name__ == '__main__':
    ...
    class TestAgent(Agent):
        def __init__(self, kernel_service: KernelService):
            super().__init__("TestAgent", kernel_service, skills_plugin_name="TestPlugin")

        async def run(self, task_description: str, current_step_inputs: Dict[str, Any], shared_context: SharedContext) -> Dict[str, Any]:
            self.logger.info(f"{self.agent_name} received task: {task_description} with inputs: {current_step_inputs}") # Use self.logger
            self.logger.info(f"Operating with SharedContext ID: {shared_context.get_session_id()}")
            kernel = self.get_kernel()
            self.logger.info(f"Kernel instance obtained in TestAgent: {type(kernel)}") # Use self.logger

            # Test getting its own plugin
            plugin = self.get_plugin(self.skills_plugin_name)
            if plugin:
                 logger.info(f"Successfully retrieved plugin '{self.skills_plugin_name}' for {self.agent_name}.")
            else:
                 logger.warning(f"Could not retrieve plugin '{self.skills_plugin_name}' for {self.agent_name}.")

            return {"status": "success", "message": "TestAgent executed", "agent": self.agent_name}
    ...
    async def main():
        ...
        logger.info(f"TestAgent run result: {result}") # Use global logger or TestAgent's logger for this line if preferred.
