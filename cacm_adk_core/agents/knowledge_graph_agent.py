...
class KnowledgeGraphAgent(Agent):
    ...
    async def run(self, task_description: str, current_step_inputs: Dict[str, Any], shared_context: SharedContext) -> Dict[str, Any]:
        ...
        company_uri_base = current_step_inputs.get("company_uri_base", "http://example.com/entity/")
        ...
        if kg_file_path_input:
            ...
            if os.path.exists(kg_file_to_load):
                try:
                    self.logger.info(f"Loading Knowledge Graph from file: {kg_file_to_load}")
                    graph.parse(kg_file_to_load, format="turtle")
                    initial_triples_count = len(graph)
                    kg_file_loaded_path = kg_file_to_load
                    self.logger.info(f"KG loaded from file. Initial graph size: {initial_triples_count} triples.")
                except Exception as e:
                    self.logger.error(f"Error loading KG file {kg_file_to_load}: {e}")
                    agent_final_status = "warning"
                    agent_final_message = f"Error loading KG file {kg_file_to_load}. Error: {e}"
            else:
                self.logger.warning(f"Specified KG file {kg_file_to_load} not found.")
                if agent_final_status == "success": agent_final_status = "warning" # Only downgrade if no prior error
                agent_final_message = f"KG file {kg_file_to_load} not found, but proceeding."


        triples_added_count = 0
        if data_for_kg_population and isinstance(data_for_kg_population, dict):
            self.logger.info(f"Attempting KG population with provided data for company URI base: {company_uri_base}")
            population_status_summary = "Population attempted."
            try:
                kernel = self.kernel_service.get_kernel()
                if not kernel:
                    population_status_summary = "Failed: Kernel not available for KGPopulationSkill."
                    if agent_final_status == "success": agent_final_status = "error"
                else:
                    kg_pop_plugin = kernel.plugins.get("KGPopulation")
                    if not kg_pop_plugin:
                        population_status_summary = "Failed: KGPopulation plugin not found."
                        if agent_final_status == "success": agent_final_status = "error"
                    else:
                        kg_skill_func = kg_pop_plugin.get("generate_rdf_triples")
                        if not kg_skill_func:
                            population_status_summary = "Failed: 'generate_rdf_triples' function not found."
                            if agent_final_status == "success": agent_final_status = "error"
                        else:
                            kernel_args = KernelArguments(company_data=data_for_kg_population, company_uri_base=company_uri_base)
                            result = await kernel.invoke(kg_skill_func, kernel_args)
                            generated_triples = result.value

                            if isinstance(generated_triples, list):
                                self.logger.info(f"KGPopulationSkill generated {len(generated_triples)} triples.")
                                for s_str, p_str, o_str in generated_triples:
                                    try:
                                        ...
                                        self.logger.error(f"Error parsing or adding triple ('{s_str}', '{p_str}', '{o_str}'): {e_parse}")
                                ...
                                if triples_added_count != len(generated_triples):
                                     if agent_final_status == "success": agent_final_status = "partial_success"
                                     if agent_final_message == "KG operations completed.": agent_final_message = "KG population partially completed."
                            else:
                                population_status_summary = f"Failed: Skill did not return list. Result: {str(result)[:200]}"
                                if agent_final_status == "success": agent_final_status = "error"
                                if agent_final_message == "KG operations completed.": agent_final_message = "KG population failed (skill error)."
            except Exception as e_pop:
                self.logger.exception(f"Error during KG population: {e_pop}")
                population_status_summary = f"Error: {e_pop}"
                if agent_final_status == "success": agent_final_status = "error"
                if agent_final_message == "KG operations completed.": agent_final_message = f"KG population exception: {e_pop}"
        elif data_for_kg_population:
             population_status_summary = "Skipped: 'data_for_kg_population' not a dictionary."
        ...
        if sparql_query:
            ...
            if len(graph) == 0 and kg_file_loaded_path == "None" and triples_added_count == 0:
                self.logger.warning("Attempting to query an entirely empty graph. Results will be empty.")
                query_results_list = []
...
if __name__ == '__main__':
    logging.basicConfig(level=logging.DEBUG) # Use DEBUG for more verbose output from agent
    ...
    sample_company_data_for_kg = {
        ...
        "financial_data_for_ratios_expanded": {
            "current_assets": 800000.0, "total_debt": 450000.0, "source": "test_suite"
        },
        "altdata_utility_payments": [
            {"utilityType": "Telecom", "paymentStatus": "on-time", "paymentDate": "2023-07-01"}
        ],
        "esg_overall_rating": {"ratingValue": "A+", "ratingProvider": "ESG Test Raters"}
    }
    ...
    """
    dummy_file_path = os.path.join(PROJECT_ROOT, "knowledge_graph_instantiations", "dummy_test_kg.ttl")
    ...
    async def test_run():
        ...
        result_populate = await kg_agent.run("Populate KG and Query", inputs_populate_and_query, mock_shared_context)
        print(f"KG Agent (populate & query) result: {json.dumps(result_populate, indent=2)}")
        if result_populate['status'] == 'success':
            assert len(result_populate['data']['results']) > 0, "Query should return populated data"
        ...
        result_query_only = await kg_agent.run("Query Only", inputs_query_only, mock_shared_context)
        print(f"KG Agent (query only) result: {json.dumps(result_query_only, indent=2)}")
        if result_query_only['status'] == 'success' and result_query_only['data']['results_count'] > 0:
             assert result_query_only['data']['results'][0]['label'] == "Dummy Corp from File"
             print("Assertion: Query returned data from file - PASSED")
        elif result_query_only['status'] == 'success':
             print("Warning: Query on dummy file returned no results, file might not have loaded as expected or query is wrong.")


        print("\n--- Test 3: Population Only (No SPARQL Query) ---")
        inputs_populate_only = {
            "data_for_kg_population": {**sample_company_data_for_kg, "companyTicker": "TKGP_POPONLY"}, # Change ticker for new entity
            "company_uri_base": "http://example.com/entity/",
            # No sparql_query key
        }
        result_populate_only = await kg_agent.run("Populate KG Only", inputs_populate_only, mock_shared_context)
        print(f"KG Agent (populate only) result: {json.dumps(result_populate_only, indent=2)}")
