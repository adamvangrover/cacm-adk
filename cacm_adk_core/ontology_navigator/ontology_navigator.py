...
class OntologyNavigator:
    ...
    domain ontology.
    """
    def __init__(self, ontology_path="ontology/credit_analysis_ontology_v0.3/credit_ontology_v0.3.ttl"):
        self.graph = Graph()
        self.namespaces = {}
        try:
            ...
            logger.error(f"Ontology file not found at {ontology_path}. Navigator will be empty.")
    ...
    def _bind_common_namespaces(self):
        ...
        common_ns = {
            ...
            "adkarch": Namespace("http://www.example.com/adk/architecture#") # Added new namespace
        }
    ...
    def _resolve_uri(self, term: str) -> URIRef:
        ...
        if ":" in term and not term.startswith("http"): # Likely a prefixed name
            ...
            if prefix in self.namespaces:
                ...
                logger.warning(f"Prefix '{prefix}' not found in known namespaces. Treating '{term}' as a full URI if possible.")
    ...
    def get_entity_details(self, entity_uri_or_prefixed_name: str) -> dict:
        ...
        rdf_types = [str(t) for t in self.graph.objects(subject=entity_uri, predicate=RDF.type)]
        if rdf_types:
            details["rdf_type"] = rdf_types

        # For properties, get domain and range
        if OWL.ObjectProperty in self.graph.objects(subject=entity_uri, predicate=RDF.type) or \
           OWL.DatatypeProperty in self.graph.objects(subject=entity_uri, predicate=RDF.type):
        ...
        if OWL.Class in self.graph.objects(subject=entity_uri, predicate=RDF.type):
            superclasses = [str(sc) for sc in self.graph.objects(subject=entity_uri, predicate=RDFS.subClassOf)]
            if superclasses:
    ...
    def list_classes(self, namespace_filter: str = None) -> list:
        ...
        for s, p, o in self.graph.triples((None, RDF.type, OWL.Class)):
            if isinstance(s, URIRef): # Ensure it's a URI, not a blank node
                if namespace_filter:
                    if str(s).startswith(str(self.namespaces.get(namespace_filter, namespace_filter))): # Allow full URI or prefix
                        classes.append(str(s))
                else:
                    classes.append(str(s))
        return sorted(classes)

    def find_concepts(self, keyword: str): # Kept for compatibility, simple label search
        """
