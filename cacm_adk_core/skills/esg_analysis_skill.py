...
class ESGAnalysisSkill:
    ...
    async def summarize_esg_factors_from_kg(
        ...
        """
        self.logger.info(f"Starting ESG factor summarization for {company_name} with {len(kg_query_results)} KG results.")
        ...
        for i, row in enumerate(kg_query_results):
            ...
            metric_type_full_uri = row.get("metric_type") # Full URI, e.g., http://.../esg#EnvironmentalFactor
            ...
            if metric_type_full_uri and "OverallESGRating" in metric_type_full_uri:
                rating_val = row.get("rating_value", metric_value) # Fallback to metric_value if specific key missing
            ...
            if not metric_label or metric_value is None: # metric_value can be 0 or "0"
                self.logger.warning(f"Skipping result item {i+1} due to missing label or value: {row}")
                esg_summary["processing_notes"].append(f"Skipped item {i+1}: missing label or value.")
                continue
            ...
            if metric_type_full_uri:
                ...
                metric_type_short = metric_type_full_uri.split('#')[-1] if '#' in metric_type_full_uri else metric_type_full_uri.split('/')[-1]

                if metric_type_short == "EnvironmentalFactor" or metric_type_short == "CarbonEmission" or metric_type_short == "WaterUsage": # etc.
                    esg_summary["environmental"].append(formatted_entry)
                elif metric_type_short == "SocialFactor" or metric_type_short == "EmployeeSafetyRecord": # etc.
                    esg_summary["social"].append(formatted_entry)
                elif metric_type_short == "GovernanceFactor" or metric_type_short == "BoardIndependenceRatio": # etc.
                    esg_summary["governance"].append(formatted_entry)
                else:
                    self.logger.info(f"Metric '{metric_label}' has unclassified type '{metric_type_short}'. Adding to 'other_metrics'.")
                    esg_summary["other_metrics"].append(f"{formatted_entry} (Type: {metric_type_short})")
            else: # No metric_type provided
                self.logger.info(f"Metric '{metric_label}' has no type specified. Adding to 'other_metrics'.")
                esg_summary["other_metrics"].append(formatted_entry + " (Type: Not Specified)")
        
        if not esg_summary["environmental"] and not esg_summary["social"] and \
           not esg_summary["governance"] and not esg_summary["overall_ratings"] and not esg_summary["other_metrics"]:
            esg_summary["processing_notes"].append("No specific ESG factors could be categorized from the provided KG results.")
...
if __name__ == '__main__':
    ...
    sample_kg_results_esg = [
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/carbon_emissions/2022", 
         "metric_label": "Scope 1 Carbon Emissions", "metric_value": "1500", "metric_unit": "tCO2e", 
        ...
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/water_usage/2022", 
         "metric_label": "Water Usage", "metric_value": "50000", "metric_unit": "cubic meters", 
        ...
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/employee_turnover/2022", 
         "metric_label": "Employee Turnover Rate", "metric_value": "15", "metric_unit": "%", 
        ...
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/board_independence/2022", 
         "metric_label": "Board Independence", "metric_value": "80", "metric_unit": "%", 
        ...
        {"metric_uri": "http://example.com/entity/TEC/esg_rating/overall", 
         "metric_label": "Overall ESG Score", "rating_value": "A-", "rating_provider": "ESG Corp", 
        ...
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/data_privacy_incidents/2022", 
         "metric_label": "Data Privacy Incidents", "metric_value": "0", 
         "metric_type": "http://example.com/ontology/cacm_credit_ontology/0.3/esg#SocialFactor"}, # Unit is optional
        {"metric_uri": "http://example.com/entity/TEC/esg_metric/unknown_metric/2023",
         "metric_label": "Future Sustainability Index", "metric_value": "7.5", "metric_unit": "score",
         "metric_type": "http://example.com/ontology/cacm_credit_ontology/0.3/custom#FutureMetric"} # Unclassified type
    ]
    ...
    async def main():
        ...
        summary_result = await skill.summarize_esg_factors_from_kg(sample_kg_results_esg, "Test Example Corp")
        import json
        print(json.dumps(summary_result, indent=2))

        print("\n--- Summarizing Empty KG Results ---")
        summary_empty = await skill.summarize_esg_factors_from_kg(empty_kg_results, "Test Example Corp")
        print(json.dumps(summary_empty, indent=2))
        
        print("\n--- Summarizing KG Results with Missing Data ---")
        summary_missing = await skill.summarize_esg_factors_from_kg(results_missing_data, "Test Example Corp")
