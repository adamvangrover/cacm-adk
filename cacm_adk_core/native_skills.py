...
class BasicCalculationSkill:
    ...
    def calculate_ratio(
        ...
        if not isinstance(numerator, (int, float)):
            logger.error(
                f"Invalid input type for numerator: {type(numerator)}. Must be numeric."
            )
            raise TypeError(
                f"Numerator must be a number, got {type(numerator).__name__}"
            )
        if not isinstance(denominator, (int, float)):
            logger.error(
                f"Invalid input type for denominator: {type(denominator)}. Must be numeric."
            )
            raise TypeError(
                f"Denominator must be a number, got {type(denominator).__name__}"
            )
    ...
    def simple_scorer(
        ...
        operator: Annotated[
            str, "Supported operators: '>', '<', '>=', '<=', '==', '!='. Default is '>'"
        ] = ">",
    ) -> str:
        if not isinstance(financial_metric, (int, float)):
            logger.error(
                f"Invalid input type for financial_metric: {type(financial_metric)}. Must be numeric."
            )
            raise TypeError(
                f"Financial metric must be a number, got {type(financial_metric).__name__}"
            )
        if not isinstance(threshold, (int, float)):
            logger.error(
                f"Invalid input type for threshold: {type(threshold)}. Must be numeric."
            )
            raise TypeError(
...
class FinancialAnalysisSkill:
    ...
    def calculate_basic_ratios(
        ...
        financial_data: Annotated[
            Dict[str, float],
            "A dictionary containing financial data. Expected keys: current_assets, current_liabilities, total_debt, total_equity, revenue, gross_profit, net_income, total_assets.",
        ],
        rounding_precision: Annotated[
            int, "The number of decimal places to round the results to. Default is 2."
        ] = 2,
    ) -> Dict[str, Any]:
        """
        ...
        for key, usage_example in required_keys_map.items():
            if key not in financial_data:
                ...
                logger.debug(
                    f"Optional key '{key}' (used for {usage_example}) not found in financial_data."
                )
            elif not isinstance(financial_data[key], (int, float)):
                errors.append(
                    f"Invalid type for '{key}': expected numeric, got {type(financial_data[key]).__name__}."
                )

        if (
            errors
        ):  # If type errors found, return early as calculations will likely fail
            logger.warning(
                f"Input type validation errors for calculate_basic_ratios: {errors}"
            )
        ...
        def get_numeric(key: str, ratio_name_for_error: str) -> Optional[float]:
            ...
            if value is None:
                errors.append(
                    f"Missing required data '{key}' for {ratio_name_for_error}."
                )
                return None
            if not isinstance(value, (int, float)):
                # This should have been caught by the initial type check, but as a safeguard:
                errors.append(
                    f"Invalid type for '{key}' (for {ratio_name_for_error}): expected numeric, got {type(value).__name__}."
                )
                return None
            return float(value)
        ...
        if ca is not None and cl is not None:
            if cl == 0:
                ...
                calculated_ratios_intermediate["current_ratio"] = round(
                    ca / cl, rounding_precision
                )
        ...
        if td is not None and te is not None:
            if te == 0:
                errors.append(
                    "Cannot calculate Debt-to-Equity Ratio: Total Equity is zero."
                )
            else:
                calculated_ratios_intermediate["debt_to_equity_ratio"] = round(
                    td / te, rounding_precision
                )
        ...
        if rev_gpm is not None and gp is not None:
            if rev_gpm == 0:
                errors.append("Cannot calculate Gross Profit Margin: Revenue is zero.")
            else:
                calculated_ratios_intermediate["gross_profit_margin"] = round(
                    (gp / rev_gpm) * 100, rounding_precision
                )
        ...
        if rev_npm is not None and ni_npm is not None:
            if rev_npm == 0:
                errors.append("Cannot calculate Net Profit Margin: Revenue is zero.")
            else:
                calculated_ratios_intermediate["net_profit_margin"] = round(
                    (ni_npm / rev_npm) * 100, rounding_precision
                )
        ...
        if ni_roa is not None and ta_roa is not None:
            if ta_roa == 0:
                errors.append("Cannot calculate ROA: Total Assets is zero.")
            else:
                calculated_ratios_intermediate["return_on_assets_ROA"] = round(
                    (ni_roa / ta_roa) * 100, rounding_precision
                )
        ...
        if ni_roe is not None and te_roe is not None:
            if te_roe == 0:
                errors.append("Cannot calculate ROE: Total Equity is zero.")
            else:
                calculated_ratios_intermediate["return_on_equity_ROE"] = round(
                    (ni_roe / te_roe) * 100, rounding_precision
                )
        ...
        if td_dr is not None and ta_dr is not None:
            if ta_dr == 0:
                errors.append("Cannot calculate Debt Ratio: Total Assets is zero.")
            else:
                calculated_ratios_intermediate["debt_ratio"] = round(
                    td_dr / ta_dr, rounding_precision
                )

        final_calculated_ratios = {
            k: v for k, v in calculated_ratios_intermediate.items() if v is not None
        }
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.DEBUG
    )  # Set to DEBUG to see optional key messages
    ...
    try:
        print(
            f"Ratio 10/2 = {basic_skill.calculate_ratio(numerator=10.0, denominator=2.0)}"
        )
    except Exception as e:
        print(f"Error: {e}")
    try:
        basic_skill.calculate_ratio(numerator=10.0, denominator=0.0)
    except ValueError as e:
        print(f"Ratio 10/0 Error: {e}")
    try:
        basic_skill.calculate_ratio(numerator=10.0, denominator="a")  # type: ignore
    except TypeError as e:
        print(f"Ratio 10/'a' Error: {e}")
    print(
        f"Scorer 100 > 50 = {basic_skill.simple_scorer(financial_metric=100.0, threshold=50.0, operator='>')}"
    )
    try:
        basic_skill.simple_scorer(
            financial_metric=50.0, threshold=50.0, operator="bad_op"
        )
    except ValueError as e:
        print(f"Scorer 50 bad_op 50 Error: {e}")
    ...
    good_data_expanded = {
        ...
        "current_liabilities": 500.0,  # CR = 2.0
        "total_debt": 800.0,
        "total_equity": 1200.0,  # D/E = 0.67
        "revenue": 10000.0,
        "gross_profit": 4000.0,  # GPM = 40.0
        "net_income": 1000.0,  # NPM = 10.0
        "total_assets": 5000.0,  # ROA = 20.0, Debt Ratio = 0.16
        # ROE = 83.33
    }
    print(
        f"Ratios (good expanded data): {financial_skill.calculate_basic_ratios(financial_data=good_data_expanded, rounding_precision=2)}"
    )
    ...
    print(
        f"Ratios (missing some new keys): {financial_skill.calculate_basic_ratios(financial_data=data_missing_some_new)}"
    )
    ...
    print(
        f"Ratios (zero revenue/assets): {financial_skill.calculate_basic_ratios(financial_data=data_zero_revenue_assets)}"
    )
