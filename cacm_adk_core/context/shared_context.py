...
class SharedContext:
    ...
    def __init__(self, cacm_id: str, session_id: Optional[str] = None):
        ...
        self.logger.info(
            f"SharedContext initialized for CACM ID '{self.cacm_id}' (Session ID: {self.session_id})"
        )
    ...
    def add_document_reference(self, doc_type: str, doc_uri: str):
        ...
        self.logger.info(
            f"Added document reference: Type='{doc_type}', URI='{doc_uri}'"
        )
    ...
    def add_knowledge_base_reference(self, kb_uri: str):
        if kb_uri not in self.knowledge_base_references:
            ...
            self.logger.debug(
                f"Knowledge base reference URI='{kb_uri}' already exists."
            )
    ...
    def set_global_parameter(self, key: str, value: Any):
        ...
        self.logger.info(
            f"Set global parameter: Key='{key}' (Value type: {type(value).__name__})"
        )  # Added type
    ...
    def set_data(self, key: str, value: Any):
        ...
        self.logger.info(
            f"Set data in data_store: Key='{key}' (Value type: {type(value).__name__})"
        )
